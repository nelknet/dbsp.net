...
[<FsCheck.NUnit.Property(MaxTest = 75)>]
let ``LSMStorage matches in-memory aggregation semantics`` (updates: (int*int*int64) list) =
    // Arrange
    ...
    let config = { DataPath = tmp; MaxMemoryBytes = 10_000_000L; CompactionThreshold = 16; WriteBufferSize = 1024; BlockCacheSize = 1_000_000L; SpillThreshold = 0.8 }
    ...
    let mem = InMemoryStorageBackend<int,int>(config) :> IStorageBackend<int,int>

    // Act
    (lsm.StoreBatchWithFlush(updates, true)).GetAwaiter().GetResult()
    (mem.StoreBatch(updates)).GetAwaiter().GetResult()

    let lsmSeq = ((lsm :> IStorageBackend<int,int>).GetIterator()).GetAwaiter().GetResult() |> Seq.toArray |> Array.sort
    let memSeq = (mem.GetIterator()).GetAwaiter().GetResult() |> Seq.toArray |> Array.sort
...
type StoragePropertyTests_Remainder() =
    let configFsCheck = Config.Quick

    [<Test>]
    member _.``Order of updates does not affect final state``() =
        let property (updates: (int*int*int64) list) =
            ...
            a = b
        let arb = Arb.fromGen StorageGenerators.updateGen
        Check.One(configFsCheck, Prop.forAll arb property)

    [<Test>]
    member _.``Compaction is idempotent and preserves results``() =
        let property (updates: (int*int*int64) list) =
    ...
    [<Test>]
    member _.``No zero-weight entries remain after updates and compaction``() =
        let property (updates: (int*int*int64) list) =
    ...
    [<Test>]
    member _.``TemporalSpine compaction preserves QueryAtTime results``() =
        let property (updatesByTime: (int64 * (int*int*int64) list) list) =
