...
type MapOperator<'I, 'O>(transform: 'I -> 'O, ?name: string) =
...
type ZSetMapOperator<'K1, 'K2 when 'K1: comparison and 'K2: comparison>(transform: 'K1 -> 'K2, ?name: string) =
...
type FilterOperator<'T>(predicate: 'T -> bool, ?name: string) =
...
type ZSetFilterOperator<'K when 'K: comparison>(predicate: 'K -> bool, ?name: string) =
...
type FlatMapOperator<'I, 'O>(transform: 'I -> seq<'O>, ?name: string) =
...
type ZSetFlatMapOperator<'K1, 'K2 when 'K1: comparison and 'K2: comparison>(
    transform: 'K1 -> seq<'K2>, ?name: string) =
